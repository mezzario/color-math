...
export default class LessEvaluator extends EvaluatorBase {
    ...
    evalNumberLiteral(node) {
        let n = this.core.evalNumberLiteral(node)
    ...
    evalArrayLiteral(node) {
        let value = node.value.map(expr => expr.evaluate(this)).join(" ")
    ...
    evalBrewerConst(node) {
        let coreValue = this.core.evalBrewerConst(node)
        let value = coreValue.map(c => c.hex()).join(" ")
    ...
    evalColorsMix(node) {
        ...
        let mode = (node.options || {}).mode
        if (mode && mode !== "rgb")
            Utils.throwError("LESS supports mixing colors only in RGB color space", node.$loc)

        let func = "mix"
        let leftColorStr = node.left.evaluate(this.core).hex("rgba").toLowerCase()
