...
describe('CoreEvaluator', () => {
  ...
  it('should support operations with color scales', () => {
    expr('scale (red 0f0 blue) -> 10', '[#ff0000, #c63900, #8e7100, #55aa00, #1ce300, #00e31c, #00aa55, #00718e, #0039c6, #0000ff]')
    expr('scale (yellow 008ae5) -> 10', '[#ffff00, #e3f219, #c6e533, #aad84c, #8ecb66, #71be7f, #55b199, #39a4b2, #1c97cc, #008ae5]')
    expr('scale (t 2000 t 6000) -> 10', '[#ff8b14, #ff972c, #ffa245, #ffae5d, #ffb975, #ffc58e, #ffd0a6, #ffdcbe, #ffe7d7, #fff3ef]')
    ...
    expr('scale (red:.2 0f0:50%) -> 10', '[#ff0000, #e31c00, #c63900, #aa5500, #8e7100, #718e00, #55aa00, #39c600, #1ce300, #00ff00]')
    expr('scale (red 0f0) @domain (.2 .5) -> 10', '[#ff0000, #e31c00, #c63900, #aa5500, #8e7100, #718e00, #55aa00, #39c600, #1ce300, #00ff00]')
    expr('scale (red 0f0) @pad .25 -> 10', '[#bf4000, #b14e00, #a35c00, #956a00, #877800, #788700, #6a9500, #5ca300, #4eb100, #40bf00]')
    expr('bezier (red 0f0) @pad (.1 .3) -> 10', '[#f74a00, #f16000, #ea7200, #e38200, #db9100, #d29e00, #c9ab00, #beb700, #b2c300, #a4ce00]')

    expr('cubehelix @pad (0 .2) -> 10', '[#000000, #19122b, #18334b, #175a49, #377434, #767b33, #b5795e, #d383a5, #cea1df, #c1caf3]')
    expr('cubehelix @start 200 -> 10', '[#000000, #072818, #2e4610, #794b2a, #a8547c, #9c7bc8, #86b4d5, #a0dbbf, #ddeacb, #ffffff]')
    expr('cubehelix @rot (-.5) -> 10', '[#000000, #29122f, #432c62, #514d8d, #5a72ac, #6699bd, #7cbcc6, #9fd9ce, #cdefde, #ffffff]')
    expr('cubehelix @hue .5 -> 10', '[#000000, #1b1a29, #274044, #406247, #747652, #a7847e, #bf9db8, #c5c3dc, #d8e7e9, #ffffff]')
    expr('cubehelix @hue (1 0) -> 10', '[#000000, #1b1932, #1d444a, #396742, #74764f, #a48580, #b8a1b3, #c6c5d0, #e0e4e4, #ffffff]')
    expr('cubehelix @gamma .7 -> 10', '[#000000, #343061, #2a6b76, #489457, #959a53, #d7978d, #e3aad7, #d3d1f8, #dcf1f4, #ffffff]')
    expr('cubehelix @lt (.3 .8) -> 10', '[#743467, #565294, #377d87, #489558, #8a8e46, #c67f75, #cd86bf, #aca8e6, #97cdd6, #aedfb8]')
    expr('cubehelix @start 200 @rot .5 @gamma .8 @lt (.3 .8) -> 10', '[#3b6c8f, #5871aa, #7874bc, #9978c5, #b77fc6, #cf89c1, #e097ba, #e9a9b5, #ecbcb4, #ebcfbb]')

    expr('+scale (black red yellow) -> 10', '[#000000, #440000, #6f0000, #9e0000, #cf0000, #ff1800, #ff7400, #ffa800, #ffd500, #ffff00]')
    expr('+bezier (yellow 0f0 blue) -> 10', '[#ffff00, #c3f336, #aade5a, #9ec676, #95ad8f, #8d93a6, #8178bc, #715cd2, #553be9, #0000ff]')
  })
